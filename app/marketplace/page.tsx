          ...
          {filtered.map((t:any)=>(
            <div key={t.id} className="bg-white rounded-[20px] p-5 border-2 border-black/10 shadow-sm">
              <Link href={`/trip/${t.id}`} className="block">
                ...
                <p className="text-[12px] font-bold text-black/60 mt-1">{t.date} • {t.time} • Hilux • Plate {t.plate}</p>
                <p className="font-black text-[24px] text-black mt-3">MK {Number(t.seat_price).toLocaleString()}</p>
                <div className="mt-4 h-[240px] rounded-[14px] overflow-hidden bg-gray-100 border border-black/5">
                  <img src={t.vehicle_url} className="w-full h-full object-cover"/>
                </div>
              </Link>
              <a href={`https://wa.me/${t.whatsapp?.replace(/[^0-9]/g,'')}`} target="_blank" className="mt-4 w-full bg-[#22c55e] text-white font-black py-4 rounded-full flex justify-center text-[14px]">WhatsApp {t.driver_name?.split(' ')[0]}</a>
            </div>
          ))}
        </div>
      </div>
