      ...
      <div className="max-w-[1200px] mx-auto p-4 md:p-8 bg-white">
        ...
        <div className="mt-6 flex flex-col md:flex-row gap-3 md:items-center">
          ...
          <div className="flex gap-2 bg-[#f6f7f9] p-1 rounded-full w-fit">
            ...
            <button onClick={()=>setTab('rides')} className={`px-6 py-2.5 rounded-full font-black text-[13px] ${tab==='rides'?'bg-black text-white':'text-black/60'}`}>Rides</button>
          </div>
        </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredTrips.map((t:any)=>(
              <div key={t.id} className="bg-white rounded-[20px] p-5 border-2 border-black/10 shadow-sm">
                ...
                <a href={`https://wa.me/${t.whatsapp?.replace(/[^0-9]/g,'')}`} target="_blank" className="mt-4 w-full bg-[#22c55e] text-white font-black py-4 rounded-full flex justify-center">WhatsApp</a>
              </div>
            ))}
          </div>
        )}
