...
export default function DriverPage(){
  ...
  return (
    <main className="max-w-[720px] mx-auto p-4 pb-20">
      ...
      <div className="mt-6 bg-white border rounded-[24px] p-6">
        <div className="flex gap-4 items-center">
          ...
          <div>
            ...
            <p className="text-[12px] opacity-60">ID: {id.slice(0,8)} • Joined 2026</p>
        ...
        <div className="grid grid-cols-3 gap-3 mt-6">
          ...
          <div className="bg-black/5 rounded-2xl p-3 text-center"><div className="font-black text-xl">{reviews.length || 1}</div><div className="text-[11px] font-bold opacity-60">BOOKINGS</div></div>
      ...
      <div className="mt-4 space-y-3">
        {trips.map((t:any)=>(
          <div key={t.id} className="bg-white border rounded-[20px] p-4 flex justify-between items-center"><div><div className="font-black">{t.from_city} → {t.to_city}</div><div className="text-[12px] opacity-60">{t.date} • {t.seats} seats • MK {t.price}</div></div><a href={`https://wa.me/${driver.phone?.replace(/\D/g,'')}?text=Booking ${t.from_city} to ${t.to_city}`} className="bg-black text-white px-5 py-2 rounded-full text-[12px] font-black">Book</a></div>
        ))}
        {trips.length===0 && <p className="text-[13px] opacity-60">No active trips</p>}
      </div>

      <h2 className="font-black mt-10 text-[18px]">Reviews ⭐ {avg} ({reviews.length})</h2>
      <div className="mt-3 space-y-3">
        {reviews.map((r:any)=>(
          <div key={r.id} className="bg-white border rounded-[20px] p-4"><div className="flex justify-between"><span className="font-black text-[13px]">{r.author_name}</span><span className="text-[11px]">{"⭐".repeat(r.rating)}</span></div><p className="text-[13px] mt-1 opacity-70">"{r.comment}"</p><p className="text-[11px] mt-1 opacity-40">{new Date(r.created_at).toLocaleDateString()} • {r.trip_route}</p></div>
        ))}
        {reviews.length===0 && <p className="text-[13px] opacity-60">No reviews yet — be first!</p>}
      </div>

      <div className="mt-8 bg-white border rounded-[24px] p-5">
        ...
        <form onSubmit={submitReview} className="mt-3 space-y-3">
          ...
          <select value={rating} onChange={e=>setRating(parseInt(e.target.value))} className="w-full border rounded-full px-4 py-3 text-[13px] font-bold"><option value={5}>⭐⭐⭐⭐⭐ 5</option><option value={4}>⭐⭐⭐⭐ 4</option><option value={3}>⭐⭐⭐ 3</option><option value={2}>⭐⭐ 2</option><option value={1}>⭐ 1</option></select>
