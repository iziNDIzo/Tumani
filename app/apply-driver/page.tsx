...
export default function ApplyDriver(){
  ...
  const sendOtp = ()=>{ if(phone.length<9) return alert("Enter valid phone"); setStep(2) } // demo: code is 1234
  ...
  const uploadFile = async (file:File, name:string)=>{
    ...
    const {data} = supabase.storage.from("driver-docs").getPublicUrl(path) // private but we store path
    // store path, not public url for private bucket
    return path
  }
