...
export default function AuthPage(){
  ...
  const handleLogin = async () => {
    ...
    try{
      ...
      router.push("/") // go to marketplace after login, not /driver
  ...
  const handleCreate = async () => {
    ...
    try{
      ...
      if(!userId) throw new Error("Disable email confirm in Supabase > Auth > Providers > Email")
    ...
    return (
      <div className="min-h-[calc(100vh-64px)] bg-[#fcfcfc] flex items-center justify-center p-4">
        <div className="w-full max-w-[520px] bg-white rounded-[32px] border border-black/10 p-10 text-center shadow-[0_20px_60px_rgba(0,0,0,0.06)]">
          ...
          <p className="text-black/60 font-medium mt-3">Welcome to Tumani Driver, {form.name.split(' ')[0]}!</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-[calc(100vh-64px)] bg-[#fcfcfc] flex items-center justify-center p-4">
      <div className="w-full max-w-[520px] bg-white rounded-[32px] border border-black/10 shadow-[0_20px_60px_rgba(0,0,0,0.06)] p-8 md:p-10">
        ...
        <div className="flex bg-black/5 rounded-full p-1 mb-8">
          <button onClick={()=>{setMode('login'); setStep(1); setErrorMsg("")}} className={`flex-1 h-10 rounded-full font-black text-[14px] ${mode==='login'?'bg-black text-white':'text-black/60'}`}>Login</button>
          <button onClick={()=>{setMode('signup'); setStep(1); setErrorMsg("")}} className={`flex-1 h-10 rounded-full font-black text-[14px] ${mode==='signup'?'bg-black text-white':'text-black/60'}`}>Become a Driver</button>
        </div>

        {mode === 'login'? (
          <div>
            <h1 className="font-black text-[32px] leading-[0.95]">Welcome back 👋</h1>
            <p className="text-black/60 font-medium mt-2">Login to manage your trips</p>
            <div className="space-y-5 mt-8">
              <div><label className="text-[11px] font-black tracking-[0.15em] text-black/40">EMAIL</label><input value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="you@gmail.com" className="mt-2 w-full h-[56px] px-5 rounded-2xl bg-[#f5f3ff] border border-black/10 outline-none font-bold" /></div>
              <div><label className="text-[11px] font-black tracking-[0.15em] text-black/40">PASSWORD</label><input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="••••••••" className="mt-2 w-full h-[56px] px-5 rounded-2xl bg-[#f5f3ff] border border-black/10 outline-none font-bold" /></div>
              {errorMsg && <div className="bg-red-50 border border-red-200 text-red-600 text-[13px] font-bold p-3 rounded-xl">{errorMsg}</div>}
              <button onClick={handleLogin} disabled={loading} className="w-full h-[60px] bg-[#1a73e8] text-white rounded-full font-black text-[16px] disabled:opacity-50">{loading? "Logging in..." : "Login →"}</button>
              <p className="text-center text-[13px] text-black/40 font-bold">Customer? You don't need to login, just <a href="/" className="text-black underline">Find Trips</a> and book.</p>
            </div>
          </div>
        ) : (
          <>
