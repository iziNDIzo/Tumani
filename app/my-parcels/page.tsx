...
export default function MyParcels(){
  ...
  return(
    <div className="max-w-[600px] mx-auto p-4">
      ...
      <div className="mt-4 space-y-3">
        ...
        {parcels.map(p=>(
          <div key={p.id} className="bg-white p-4 rounded-[20px] border">
            <div className="flex justify-between"><span className="font-black text-[14px]">{p.tracking_id}</span><span className="bg-black text-white px-3 py-1 rounded-full text-[11px] font-bold">{p.status}</span></div>
      ...
      <a href="/send-parcel" className="mt-6 block w-full h-[50px] bg-blue-600 text-white rounded-full flex items-center justify-center font-black">Send Another Parcel</a>
    </div>
  )
}